...
JSHG.resetVariables_ = function() {
    ...
    JSHG.settings_["workerFilePath"] = "video-gesture/gestureRecognizer.js";
...
JSHG.startVideo_ = function() {
    try{
        compatibility.getUserMedia({video: true}, function(stream) {
            try {
                JSHG.video_.attr("srcObject", compatibility.URL.createObjectURL(stream));
            } catch (error) {
                JSHG.video_.attr("srcObject", stream);
            }
            setTimeout(function() {
                JSHG.video_[0].play();
